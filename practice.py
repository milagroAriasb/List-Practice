...
def halvesies(numbers):
    ...
    for number in numbers:
        halve = number/2.0
        halvesies_results.append (halve)

    return halvesies_results
...
def join_strings(words):
    ...
    #for word in words:
    for word in words: 
        all_words_together += '%s' % (word)
    

    return all_words_together
...
def average(numbers):
    ...
    average_amount = sum_amount/len(numbers)
...
def join_strings_with_comma(words):
    ...
    all_words_join_comma= ''
    #for word in words:
    for word in words: 
        all_words_join_comma += '%s, ' % (word)


    return all_words_join_comma[:-1]
...
def reverse_list(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    You should do this without changing the original list::
        ...
        ['apple', 'berry', 'cherry']
    """
    items_in_reverse_order = items[len(items)::-1]
    return items_in_reverse_order
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:
            if letter in word:
                letter_index.append(word.index(letter))  # I have used index() before so I looked it up in python library online
            else: 
